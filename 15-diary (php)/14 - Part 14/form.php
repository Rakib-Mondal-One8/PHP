...
if (!empty($_POST)) {
    ...
    if (!empty($_FILES) && !empty($_FILES['image'])) {
        if ($_FILES['image']['error'] === 0 && $_FILES['image']['size'] !== 0) {
            ...
            $destinationImg = __DIR__ . '/uploads/' . $imageName;

            $imageSize = getimagesize($originalImg);
            if(!empty($imageSize)){
    ...
    $stmt = $pdo->prepare('INSERT INTO `entries` (`title`,`date`,`message`,`image`) VALUES (:title,:date,:message,:imageName)');
    // var_dump($stmt);
    // var_dump($date);
